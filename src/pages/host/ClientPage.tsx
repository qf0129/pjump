...
export default function ClientPage() {
  ...
  const clientView = () => {
    ...
    if (connProtocol === 'vnc' || connProtocol === 'rdp')
      return <GuacdClient hostUid={host.Uid!} osUserUid={connOsUserUid} protocol={connProtocol} />;
          ...
          <Form.Item label="系统用户">
            {osUsers.length ? (
              <Radio.Group value={selOsUserUid} onChange={(e) => setSelOsUserUid(e.target.value)} buttonStyle="solid">
                {osUsers.map((u) => (
                  <Radio.Button key={u.Uid} value={u.Uid}>{`${u.Name || u.Username} (${u.Username})`}</Radio.Button>
                ))}
              </Radio.Group>
            ) : (
