...
export default function HostPage() {
  ...
  const handleDeleteConfirm = () => {
    ...
    PersonalApi.DeleteHost({ Uid: deleteTarget.Uid!, DeleteOsUser: deleteOsUser }).then((res) => {
      if (res.Code === 0) {
  ...
  return (
    <div style={{ padding: 24, height: "100%", overflow: "auto", display: "flex", flexDirection: "column" }}>
      <Flex align="center" justify="space-between" gap={12} style={{ marginBottom: 24 }}>
        ...
        <Input.Search placeholder="搜索名称、IP…" allowClear value={search} onSearch={handleSearch} onChange={(e) => setSearch(e.target.value)} style={{ maxWidth: 360 }} />
      </Flex>
